<?php
require APPPATH. '/controllers/Helpdesk_forcall/Helpdesk_forcall_config.php';

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Helpdesk_forcall extends CI_Controller {
   private $log_key,$log_temp,$title;
   function __construct(){
        parent::__construct();
		$this->load->model('Helpdesk_forcall/Helpdesk_forcall_model','tmodel');
		$this->log_key ='log_Helpdesk_forcall';
		$this->title = new Helpdesk_forcall_config();
   }
	...
	public function index(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_refresh_table'	=> site_url().'Helpdesk_forcall/Helpdesk_forcall/refresh_table/'.$this->_token,
			'link_create'			=> site_url().'Helpdesk_forcall/Helpdesk_forcall/create',
			'link_update'			=> site_url().'Helpdesk_forcall/Helpdesk_forcall/update',
			'link_delete'			=> site_url().'Helpdesk_forcall/Helpdesk_forcall/delete_multiple',
		);
		
		$this->template->load('Helpdesk_forcall/Helpdesk_forcall_list',$data);
	}
	...
	public function create(){
		$data = array(
			...
			'title'					=> $this->title,
			'link_save'				=> site_url().'Helpdesk_forcall/Helpdesk_forcall/create_action',
			'link_back'				=> $this->agent->referrer(),			
		);
		
		$this->template->load('Helpdesk_forcall/Helpdesk_forcall_form',$data);
	...
	public function create_action(){
		...
		*/	
		...
		if(!$o->not_empty($val['umur'],'#umur')){
			echo $o->result();	
			return;
		}

		//mencegah data double
		$field=array('umur'=>$val['umur']);
		$exist = $this->tmodel->if_exist('',$field);
		if(!$o->not_exist($exist,'#umur')){
			echo $o->result();	
			return;
		}
		...
		**/
		...
		if($row){
			$data = array(
				'title_page_big'		=> 'Buat Baru',
				'title'					=> $this->title,
				'link_save'				=> site_url().'Helpdesk_forcall/Helpdesk_forcall/update_action',
				'link_back'				=> $this->agent->referrer(),
				'data'					=> $row,
				'id'					=> $id_generate,
			);
