...
class Report_helpdesk extends CI_Controller
{
	...
	public function index()
	{
		...
		$response['chanel_petugas'] = $this->dapros->live_query("SELECT agentid,CHANEL,status,count(*) as numna FROM `dapros_helpdesk_va` WHERE status > 0 AND DATE(tgl_distribution)>='$start' AND DATE(tgl_distribution)<='$end' GROUP BY agentid,CHANEL,status ")->result();
